...
class Prediction(object):
    ...
    def prepare_datasets(self, dbmanager, toolbox):
        ...
        for pkfd in peakfiles_lst:
            ...
            if (self.config['compfeat']).lower() in ['y', '1', 'yes', 'oh yeah', 'please', 'sure', 'of course', 'positive']:
                tuplist = self._prepare_feature_computation(merged_lst, pkfd, self.config['features'], dbmanager, toolbox)
                newdbentry_noid = self._create_prelim_metadata(pkfd, self.config['features'], "genomicregions", len(merged_list))
    ...
    def _check_existing(self, peakfiles_dict, datasets_dict):
        ...
        """
        existing = set([ int((d['link']).strip("ASSOC_")) for d in datasets_dict.values() ])
    ...
    def _get_signal_intersect(self, fileid, dbmanager, toolbox):
        ...
        for e in entries:
            ...
            perc = {}
            # this is list of ids
            signal_files = e['intersectfiles'].split("-")
    ...
    def _get_peak_intersect(self, fileid, dbmanager, toolbox):
        ...
        result = []
        for e in entries:
            peak_files = e['intersectfiles'].split("-")
            for p in peak_files:
                try:
