...
class BEDFeatures(object):
    ...
    def _GC_CpG_content(self, d):
        try:
            ...
            d['perc_GC'] = round(((total_G + total_C) / seqlen) * 100., 3)
            d['perc_CpG'] = round((total_CpG / (seqlen / 2.)) * 100., 3)
            d['obsexp_CpG'] = round((total_CpG / (max(1,total_G) * max(1,total_C))) * seqlen, 3)
            return d
        ...
        if all_sigfiles:
            for sigovl in all_sigfiles:
                ...
                try:
                    ...
                    keyname = keyname + "_"
                    p20, p40, p60, p80, p90, p95 = list(map(float, [perc['p20'], perc['p40'], perc['p60'], perc['p80'], perc['p90'], perc['p95']]))
